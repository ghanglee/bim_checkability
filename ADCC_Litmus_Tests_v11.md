# ADCC BIM-Checkability Litmus Tests (v11)

July 27, 2026

## Evaluation order

When the seven properties are scored under short-circuit evaluation, they are assessed in this
fixed order and evaluation stops at the first failure, which is recorded as the reason:
...
Sentential completeness is first because referential and normative completeness both presuppose
it. Referential precedes normative because it is the more reliably labelled of the two
(0.949 vs 0.876 for the best-performing model; the ordering holds for every model tested).
Process and Data precede Human so that a defect already charged to an external process or a
missing datum is not charged a second time to human judgment (LT10). Atomicity is last because
it is scored only once the other six pass (LT4).

Because the verdict is a conjunction, the order does not change any verdict. It changes only
which property is recorded as the reason on a statement that fails more than one, and how many
properties are scored before stopping.

**Changed in v10:** the evaluation order is now stated explicitly and places Referential before
Normative; LT2, LT3 and the decision sequence list their leaves in evaluation order; and the LT10b
line of the decision sequence, which previously labelled the formal-procedure branch as Human, is
corrected to Process. No test's content changed.
...
**Related properties:**
Completeness, Sentential, Referential, Normative

**Problem:**
"Completeness" is a summary label; graders need the three checks it stands for.

**Litmus test question:**
Are Sentential, Referential, and Normative all Complete? Yes → Completeness = Complete. Any one fails → Incomplete. When scored under short-circuit evaluation they are assessed in that order.
...
**Related properties:**
Self-Sufficiency, Process, Data, Human

**Problem:**
Same issue as LT2, for the other parent axis.

**Litmus test question:**
Are Process, Data, and Human all Independent? Yes → Self-Sufficiency = Yes. Any one Dependent → No. When scored under short-circuit evaluation they are assessed in that order, so a defect already charged to Process or Data is not charged again to Human.
...
**Problem:**
Once Sentential or Normative already fails, Process/Data/Human can look Dependent for no real reason — there was never a genuine question to check.

**Litmus test question:**
Is there still a real predicate in the sentence — a precondition, exception, or hidden criterion — that would need checking if the rest of the row were fixed? No → Process = Data = Human = Independent. Yes → score them normally.
...
## Decision sequence (short-circuit evaluation)

```
Sentential incomplete? (LT6) ─────────────────────────────► No (Sentential)
Referent outside the corpus? (LT8/LT8a) ──────────────────► No (Referential — or Data, via LT8)
Not a pass/fail obligation? (LT7) ────────────────────────► No (Non-normative / P / Compound)
Compliance is a real-world event, or verifying it needs
  a formal/structured procedure? (LT9/LT10b) ─────────────► No (Process)
Open criterion missing or external? (LT10/LT10a) ─────────► No (Data)
Predicate not formalizable, or confirmable only by a
  person observing directly? (LT10/LT10a/LT10b) ──────────► No (Human)
More than one rule? (LT12/LT12a) ─────────────────────────► No (Compound — split)
else ─────────────────────────────────────────────────────► YES (BIM-checkable)
```

The sequence is Sentential → Referential → Normative → Process → Data → Human → Atomicity, as set
out under "Evaluation order" above. Note the direction of LT10b: needing a formal procedure makes a
statement **Process**-dependent, while one a person can confirm by walking in and observing is
**Human**-dependent. An earlier version of this block had those two the wrong way round.

*Property map:* Completeness = Sentential ∧ Referential ∧ Normative (LT2). Self-Sufficiency = Process ∧ Data ∧ Human (LT3), short-circuited to Independent once Sentential/Normative already failed and nothing substantive remains to check (LT3a). Atomicity assessed only when the other six pass (LT4, NA-gate). **Verdict = Yes ⟺ all seven pass (LT1).**
